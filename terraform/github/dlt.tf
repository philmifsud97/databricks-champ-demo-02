module "dlt_user" {
  ...
  main_pipeline_repo_path = databricks_repo.demo_in_user_home.path
  test_pipeline_name      = "${var.dlt_name_prefix} Integration Test (Dev, ${data.databricks_current_user.me.alphanumeric})"
  test_pipeline_repo_path = databricks_repo.demo_in_user_home.path
}
...
module "dlt_prod" {
  ...
  main_pipeline_repo_path      = databricks_repo.demo_in_prod.path
  test_pipeline_name           = "${var.dlt_name_prefix} Integration Test (Staging, ${data.databricks_current_user.me.alphanumeric})"
  test_pipeline_repo_path      = databricks_repo.demo_in_staging.path
}
